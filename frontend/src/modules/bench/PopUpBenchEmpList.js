...
let PopBenchEmpList = () => {
  ...
  let handleBenchEmployeeSubmit = (event) => {
    ...
    if (
      ...
      !teamLead &&
      !notes &&
      !primarySkills.skillName && !primarySkills.totalExp &&
      !reportingManager
    ) {
  ...
  let handleUpdateBechEmployee = (item) =>{
    setIsEdit(true);
    console.log(item)
    Axios.get(`http://localhost:5000/EmployeeList/${item.id}`).then((res)=>{
      setBench({
        id:item.id,
        name:item.name,
        email:item.email,
                      ...
                      <div className="col-md-6">
                        <div className="md-form mb-0">
                          <input
                            ...
                            required
                          />
                          <div class="invalid-feedback">Please enter EmpId</div>
                        </div>
                      </div>
                    </div>
                    <div className="row mb-3">
                  ...
                  benchList.map((item) => {
                    ...
                    return (
                      <tr key={item.id}>
                        ...
                        <td>{item.teamLead}</td>
                        <td>
                          <div data-toggle="modal" data-target="#modalContactForm"><i className="fas fa-edit" onClick={()=>handleUpdateBechEmployee(item)}/></div>  
                          &nbsp;&nbsp;
                          <i
...
export default PopBenchEmpList;
